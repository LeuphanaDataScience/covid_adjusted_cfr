...
## useful for age-structured infectious disease models
library(dplyr)
library(tidyr)
library(ggplot2)
...
#China (Shanghai)
#Use contact_matrix function that extracts a contact matrix from survey data
 # 9 groups
...
# data.frame(contacts=as.numeric(m$matrix)) %>% #make a column contacts with all the values
#   tbl_df() %>% # make a tibble
...
EU_cm  # The final contact matrix for all europe 
...
CN_cm = gather(contacts,"j","c",1:9) %>%
  ...
  scale_fill_gradient(low="darkblue",high="darkgoldenrod1") +
  labs(x="Age group",y="Age group",fill="Contacts")
CN_cm  # The final contact matrix for China
