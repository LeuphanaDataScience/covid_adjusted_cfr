...
# Contact matrix: (https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1005697#sec020)
  # data/contact_matrix_prems2017/MUestimates_all_locations_1.xlsx : A
  # data/contact_matrix_prems2017/MUestimates_all_locations_2.xlsx : N

# Age distribution:(http://data.un.org/Data.aspx?d=POP&f=tableCode%3A22)
 # data/age_distribution.csv(used to transform contact matrix)
...
get_sym_contact_matrix10=function(country){
  #load age distribution, 5-year group (as contact matrix is split in 5-year groups)
  excel_age_dist=ifelse(country=="China",
                        "data/contact_matrix/age_distribution_china.csv",
                        "data/contact_matrix/age_distribution.csv")
  ...
  if(country %in% country_excel1){
    contact <- as.data.frame(((read_excel("data/contact_matrix/contact_matrix_prems2017/MUestimates_all_locations_1.xlsx",
                                          sheet=country,col_names=TRUE))))
  }else if(country %in% country_excel2){
    contact <- as.data.frame(((read_excel("data/contact_matrix/contact_matrix_prems2017/MUestimates_all_locations_2.xlsx",
                                          sheet=country,col_names=FALSE))))
  }else{
...
m<-get_sym_contact_matrix10("Austria")
